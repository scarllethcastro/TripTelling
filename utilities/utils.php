...
class Utilisateur {
    ...
    public static function tryDeleteUser($dbh, $user) {
        ...
        $namePhotoPosts = Post::idallposts($dbh, $username); // Nom des fichiers de photo de chaque post
...
// retorna tableau com o id de todos os posts
    public static function idallposts($dbh, $username) {
        $numposts = Post::numposts($dbh, $username);
        $sql_code = "SELECT * FROM `posts` WHERE loginuser = ?";
        $sth = $dbh->prepare($sql_code);
        $sth->setFetchMode(PDO::FETCH_CLASS, 'Post');
        $sth->execute(array($username));
        $post = $sth->fetch();
        $idposts = array($numposts);
        $i = 0;
        do {
            $idposts[$i] = $post->idpost;
            $i++;
...
// recebe como parametro todos os ids dos posts e retorna o nome das possiveis imagens de stop
    public static function imagenames($dbh, $idposts) {
        ...
        for ($i = 0; $i < $totalposts; $i++) {
            ...
            $sth = $dbh->prepare($sql_code);
            $sth->setFetchMode(PDO::FETCH_CLASS, 'Stop');
            $sth->execute(array($idposts[$i]));
            $stop = $sth->fetch();
            do {
                $size = count($nameimages);
                $size++;
                $nameimages = array_pad($nameimages, $size, $idposts[$i] . "." . $stop->idstop . "." . "0");
                $size++;
                $nameimages = array_pad($nameimages, $size, $idposts[$i] . "." . $stop->idstop . "." . "1");
                $size++;
                $nameimages = array_pad($nameimages, $size, $idposts[$i] . "." . $stop->idstop . "." . "2");
